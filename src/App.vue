...
const boardTheme = ref(localStorage.getItem('blitzdrill-boardTheme') || 'brown');
const isSidebarCollapsed = ref(localStorage.getItem('blitzdrill-sidebarCollapsed') === 'true');
...
watch(isSidebarCollapsed, (val) => localStorage.setItem('blitzdrill-sidebarCollapsed', String(val)));
...
watch(studies, (newStudies) => {
  saveStudies(newStudies);
}, { deep: true });

const handleCreateStudy = (name: string, pgn: string, defaultColor: 'white' | 'black') => {
...
const playOpponentMove = () => {
  if (currentTargetIndex.value >= currentTargetLine.value.length) {
    if (currentNode.value) {
      completedLines.value = new Set([...completedLines.value, currentNode.value.id]);
    }
  ...
  if (nextMoveNode && nextMoveNode.move) {
    ...
    if (currentTargetIndex.value === currentTargetLine.value.length) {
      completedLines.value = new Set([...completedLines.value, nextMoveNode.id]);
      ...
      <div class="sidebar-header">
        <div class="logo-row">
          <div class="logo">
            ...
            <div class="logo-text">BLITZ<span class="text-accent">DRILL</span></div>
          </div>
          <button class="sidebar-toggle" @click="isSidebarCollapsed = !isSidebarCollapsed" :title="isSidebarCollapsed ? 'Expand Sidebar' : 'Collapse Sidebar'">
            <svg v-if="isSidebarCollapsed" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="13 17 18 12 13 7"></polyline><polyline points="6 17 11 12 6 7"></polyline></svg>
            <svg v-else viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="11 17 6 12 11 7"></polyline><polyline points="18 17 13 12 18 7"></polyline></svg>
          </button>
        </div>
      ...
      <div class="sidebar-footer">
        <div class="stats" v-if="currentStudy">
          ...
          <div class="stat-item">
            ...
            <span class="stat-value">{{ Math.round((completedCount / (totalLines || 1)) * 100) }}%</span>
          </div>
        </div>
      </div>
    </aside>
    ...
    <main class="main-view">
      <header class="main-header">
        <div class="header-left">
          <h2 v-if="currentStudy" class="study-title">{{ currentStudy.name }}</h2>
        ...
        <section class="board-section">
          ...
          <transition name="slide-up">
            <div v-if="mode === 'drill' && totalLines > 0" class="drill-ui">
              ...
              <button @click="resetDrill" class="btn-ghost">Restart Session</button>
            </div>
          </transition>
        </section>
        ...
        <aside class="data-section" v-if="currentStudy">
          <div class="panel-card">
            <div class="panel-header">
              <span class="panel-title">{{ mode === 'edit' ? 'Move Tree' : 'Drill Active' }}</span>
...
.dot.correct { background: var(--accent-success); box-shadow: 0 0 8px var(--accent-success); }
.dot.variation { background: var(--accent-warning); box-shadow: 0 0 8px var(--accent-warning); }
.dot.wrong { background: var(--accent-error); box-shadow: 0 0 8px var(--accent-error); }
